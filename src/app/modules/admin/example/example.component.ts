...
export class ExampleComponent implements OnInit {
    private readonly example = inject(ExampleService);
    ...
    constructor(
        ...
        this.example.getGithubAccessToken().subscribe({
            next: (token) => {
        ...
        try {
            ...
            if (savedUrl) {
                ...
                console.log(
                    `Loaded repo URL from localStorage: ${this.repoUrl}`
                );
            }
            ...
            console.error(
                'ExampleComponent: Failed to initialize Filesystem!',
                fsError
            );
            this.addOutput(
    ...
    ngOnInit(): void {
        ...
        if (this.fs) {
            ...
            console.warn(
                'ExampleComponent: ngOnInit skipped repo check as FS failed to initialize.'
            );
        }
    ...
    async checkIfRepoExists(): Promise<void> {
        ...
        try {
            ...
            if (stat.isDirectory()) {
                this.addOutput(
                    `Repository found in ${this.dir}. Building file tree...`
                );
                this._ngZone.run(() => {
                    this.cloneDone = true;
                });
            ...
            if (e.code === 'ENOENT') {
                this.addOutput(
                    `No existing repository found in ${this.dir}. Ready to clone.`
                );
            } else {
                this.addOutput(
                    `Error checking for repository: ${e.message || e}`,
                    'error'
                );
            }
            this._ngZone.run(() => {
    ...
    async cloneRepo(): Promise<void> {
        if (!this.repoUrl || !this.repoUrl.startsWith('https://')) {
            this.addOutput(
                'Error: A valid public HTTPS Repository URL is required.',
                'error'
            );
            return;
        }
        ...
        try {
            ...
            this.addOutput(
                'Warning: Could not save repository URL to local storage.',
                'warn'
            );
        ...
        this.addOutput(
            `Attempting to clone ${this.repoUrl} into ${this.dir}...`
        );
        this._ngZone.run(() => {
            this.cloneDone = false;
            this.repositoryTree = [];
        ...
        try {
            ...
            try {
                ...
                this.addOutput(`Cleaned directory ${this.dir}.`);
            } catch (e: any) {
                if (e.code !== 'ENOENT') {
                    this.addOutput(
                        `Note: Could not automatically clean ${this.dir}. ${e.message}`,
                        'warn'
                    );
                }
            }
            ...
            try {
                ...
                this.addOutput(`Created directory ${this.dir}.`);
            } catch (mkdirErr: any) {
                if (mkdirErr.code !== 'EEXIST') {
                    throw new Error(
                        `Failed to create directory ${this.dir}: ${mkdirErr.message || mkdirErr}`
                    );
                }
            }
            ...
            await git.clone({
                ...
                singleBranch: true,
                depth: 10, // Keep clone fast
                onProgress: (progress) => {
                    /* ... progress logging ... */
                },
                onMessage: (message) => {
                    this.addOutput(`Remote: ${message.trim()}`);
                },
            });
    ...
    async buildAndDisplayFileTree(): Promise<void> {
        ...
        try {
            ...
            this.addOutput(
                `Failed to build file tree: ${error.message || error}`,
                'error'
            );
    ...
    async viewFileContent(node: TreeNode): Promise<void> {
        ...
        try {
            ...
            const content = await this.fs.promises.readFile(fsPath, {
                encoding: 'utf8',
            });
            ...
            if (content.length > maxDisplaySize) {
                ...
                this.addOutput(
                    `File content truncated for display (>${maxDisplaySize / 1024} KB).`,
                    'warn'
                );
            }
            ...
            if (
                ...
                errorMessage =
                    'Cannot display content: File appears to be binary or uses unsupported encoding.';
                this.addOutput(
                    `Cannot display content for ${node.path}: Binary or unsupported encoding.`,
                    'warn'
                );
            } else {
                this.addOutput(
    ...
    async pullChanges(): Promise<void> {
        ...
        try {
            ...
            if (fetchResult?.fetchHead) {
                this.addOutput(
                    `Fetched remote commit: ${fetchResult.fetchHead}`
                );
                this.addOutput(
                    'Step 2: Merging fetched changes (fast-forward only)...'
                );
                ...
                this.addOutput(
                    mergeResult.oid
                        ? `Pull successful (merged). New HEAD: ${mergeResult.oid}`
                        : 'Pull successful (fast-forward or already up-to-date).'
                );

                // *** REBUILD TREE AFTER PULL ***
                await this.buildAndDisplayFileTree();
            } else {
                this.addOutput('Fetch did not return a head.', 'warn');
            }
        } catch (error: any) {
            /* ... error handling ... */
        } finally {
            this.setLoading(false, 'Pull');
        }
    }
    ...
    async pushChanges(): Promise<void> {
        ...
        this.addOutput(
            'Attempting to push changes... (NOTE: Likely to fail without auth/write access)',
            'warn'
        );
        try {
            ...
            if (
                ...
                result.ok.some((s) => s?.includes('ok')) &&
                !result.error
            ) {
                this.addOutput('Push successful.', 'log');
            } else {
                this.addOutput(
                    `Push failed or encountered errors. Error: ${result?.error || 'N/A'}`,
                    'error'
                );
            ...
            this.addOutput(
                `Push failed with exception: ${error.message || error}`,
                'error'
            );
            console.error('Push Exception:', error);
        } finally {
            this.setLoading(false, 'Push');
        }
    }
    ...
    async getStatus(): Promise<void> {
        ...
        try {
            ...
            if (status.length === 0) {
                this.addOutput('  (Working directory clean)');
            } else {
                status.forEach((row) => {
                    const mapStatus = (code: number): string =>
                        ['?', 'A', 'D', 'M', 'U'][code] ?? '?'; // Shorter codes
                    this.addOutput(
                        `  '${row[0]}': ${mapStatus(row[1])}, ${mapStatus(row[2])}, ${mapStatus(row[3])}`
                    );
                });
            }
        } catch (error: any) {
