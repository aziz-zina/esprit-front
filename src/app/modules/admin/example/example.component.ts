...
export class AdminExampleComponent implements OnInit, OnDestroy {
    ...
    ngOnInit(): void {
        console.log('salem ena houni tawa');
    ...
    async commitChanges(): Promise<void> {
        if (!this.activeRepository || this.isBusy || !this.fs) {
            this.addOutput('Commit pre-conditions not met.', 'warn');
            return;
        }
        if (this.selectedCommit) {
            this.addOutput('Cannot commit while viewing history.', 'error');
            return;
        }
        if (this.selectedFile?.isDirty) {
            this.addOutput('Cannot commit: Unsaved changes exist.', 'warn');
            return;
        }
        ...
        try {
            ...
            this.addOutput(
                `Commit successful! New commit SHA: ${sha.substring(0, 7)}`,
                'log'
            );
            ...
            if (
                ...
                this.addOutput(
                    'Commit failed: No changes were staged.',
                    'warn'
                );
            }
        } finally {
