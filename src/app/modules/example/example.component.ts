...
import type { ReadObjectResult, CommitObject, PushResult } from 'isomorphic-git';
import http from 'isomorphic-git/http/web';
import { ExampleService, GithubApiRepo } from './example.service';
...
interface RepositoryInfo { id: number; name: string; fullName: string; url: string; localPath: string; }
...
export class ExampleComponent implements OnInit, OnDestroy {
    ...
    fetchGithubAccessToken(): void {
        ...
        if (this.isLoading || this.githubAccessToken) {
             if(this.isLoading) this.addOutput('Token fetch already in progress.');
             ...
             if(this.githubAccessToken && !this.isLoading) {
                 this.addOutput('Token already exists. Use "Fetch My GitHub Repos" if needed.');
            ...
            next: (token) => {
                ...
                this._ngZone.run(() => {
                    ...
                    if (token) {
                        ...
                        this.addOutput('Could not retrieve GitHub access token.', 'warn');
                    }
        ...
        this.githubApiSubscription = this.exampleService.fetchGithubRepositories(this.githubAccessToken).subscribe({
            ...
            error: (err) => {
                ...
                this._ngZone.run(() => {
                    ...
                    if (err?.status === 401) {
                        this.addOutput('Authentication error (401) fetching repos. Token might be invalid.', 'error');
    ...
    async removeManagedRepo(repoToRemove: RepositoryInfo): Promise<void> {
        if (this.isBusy) {
            this.addOutput(`Cannot remove repository while operation in progress.`, 'warn');
            return;
        }
        const confirmation = confirm(`Are you sure you want to remove the LOCAL COPY of "${repoToRemove.fullName}"? This will delete the directory ${repoToRemove.localPath} in the browser's filesystem and cannot be undone.`);
        ...
        try {
            ...
            if (pathExists) {
                ...
                this.addOutput(`Directory ${repoToRemove.localPath} not found locally. Removing from managed list only.`, 'warn');
            }
    ...
    private async deleteDirectoryRecursive(dirPath: string): Promise<void> {
        try {
            ...
            if (error.code !== 'ENOENT') {
                this.addOutput(`Warning: Could not fully delete ${dirPath}: ${error.message}`, 'warn');
    ...
    setActiveRepository(repoInfo: RepositoryInfo | null): void {
        if (this.isBusy) {
            this.addOutput('Cannot switch repository while an operation is in progress. Please wait.', 'warn');
            return;
        }
        if (this.activeRepository?.id === repoInfo?.id) { return; }
        ...
        this._ngZone.run(() => {
            ...
            this.newBranchName = '';
            this.output = [`Switched context to ${repoInfo ? repoInfo.fullName : 'none'}.`]; // Reset logs
    ...
    async checkIfRepoExists(): Promise<void> {
        ...
        try {
            ...
            this.addOutput(`Repository found locally at ${this.dir}. Loading state...`);
            ...
            if (e.code === 'ENOENT') {
                this.addOutput(`No repository found in ${this.dir}. Ready to clone.`);
                this._ngZone.run(() => { // Reset state within zone
                    this.repositoryTree = []; this.commitHistory = [];
                    this.localBranches = []; this.currentBranch = null; this.selectedFile = null;
                    this.cloneDone = false; // Explicitly set cloneDone to false
                });
            } else {
                this.addOutput(`Error checking repository state in ${this.dir}: ${e.message || e}`, 'error');
                console.error("Repo check error:", e);
                this._ngZone.run(() => { /* Reset relevant state if needed */
                    this.repositoryTree = []; this.commitHistory = [];
                    this.localBranches = []; this.currentBranch = null; this.selectedFile = null;
                    this.cloneDone = false;
                 });
    ...
    async cloneRepo(): Promise<void> {
        if (!this.activeRepository || !this.githubAccessToken || this.isLoading) return;

        this.setLoading(true, `Clone (${this.activeRepository.name})`); // Handles zone
        // Reset state within zone before starting
        this._ngZone.run(() => {
            this.repositoryTree = []; this.selectedFile = null; this.cloneDone = false;
            this.commitHistory = []; this.selectedCommit = null; this.localBranches = []; this.currentBranch = null;
            this.output = [`Attempting to clone ${this.activeRepository.fullName}...`];
        });
        ...
        try {
            ...
            this.addOutput(`Cleaning target directory ${cloneDir} before clone...`);
            ...
            await git.clone({
                fs: this.fs, http: http, dir: cloneDir, url: cloneUrl, corsProxy: this.corsProxy,
                onAuth: () => auth, singleBranch: true, depth: this.historyDepth,
                // Ensure onMessage runs inside NgZone to update the output log safely
                onMessage: (m) => { this._ngZone.run(() => this.addOutput(`Remote: ${m.trim()}`)); }
            });
            ...
            await Promise.all([ this.loadBranchData(), this.loadCommitHistory(), this.buildAndDisplayFileTree() ]);
            this.addOutput(`Initial repository state loaded after clone.`);

        } catch (error: any) {
            this.addOutput(`Clone failed: ${error.message || error}`, 'error'); console.error("Clone error:", error);
            const statusCode = error.data?.statusCode || error.http?.statusCode;
            if (statusCode === 401) { this.addOutput('Authentication failed (401). Check token.', 'error'); this._ngZone.run(() => this.githubAccessToken = null); }
            else if (statusCode === 403) { this.addOutput('Authorization failed (403). Check permissions.', 'error'); }
            else if (statusCode === 404) { this.addOutput('Repository not found (404).', 'error'); }
            else if (error.name === 'CorsError' || error.message.includes('CORS')) { this.addOutput('A CORS error occurred.', 'error'); }
    ...
    async buildAndDisplayFileTree(ref: string = 'HEAD'): Promise<void> {
        ...
        const refName = this.selectedCommit ? `commit ${targetRef.substring(0, 7)}` : ref;
        ...
        try {
            const paths = await git.listFiles({ fs: this.fs, dir: this.dir, ref: targetRef });
            ...
            this.addOutput(`Failed to build file tree for ${refName}: ${error.message || error}`, 'error');
    ...
    async pullChanges(): Promise<void> {
        if (!this.activeRepository || !this.cloneDone || this.isLoading || !this.fs || !this.githubAccessToken || this.selectedCommit) {
            this.addOutput('Pull pre-conditions not met.', 'warn'); return;
        }
        if (this.selectedFile?.isDirty) {
             if (!confirm("Discard unsaved changes and pull?")) {
                this.addOutput("Pull cancelled due to unsaved changes.", "warn"); return;
             }
              // Discard changes within zone
              this._ngZone.run(() => { if(this.selectedFile) { this.selectedFile.content = this.selectedFile.originalContent; this.selectedFile.isDirty = false; } });
        }
        ...
        try {
            ...
            const fetchResult = await git.fetch({
                fs: this.fs, http: http, dir: this.dir, onAuth: () => auth, corsProxy: this.corsProxy,
                singleBranch: true, depth: this.historyDepth, tags: false
            });

            if (fetchResult?.fetchHead && fetchResult?.fetchHead !== fetchResult?.fetchHeadDescription) {
                 this.addOutput(`Fetched remote commit: ${fetchResult.fetchHead.substring(0, 7)}`);
                 this.addOutput('Step 2: Attempting fast-forward merge...');
                 const mergeResult = await git.merge({
                     fs: this.fs, dir: this.dir, ours: 'HEAD', theirs: fetchResult.fetchHead,
                     fastForwardOnly: true,
                     // Provide dummy author, merge commit won't be created in fast-forward
                     author: { name: 'Browser User', email: 'user@browser.com' }
                 });

                 if (mergeResult.oid) { // Success
                    this.addOutput(`Pull successful. Local branch updated to ${mergeResult.oid.substring(0, 7)}`);
                    // Refresh state - these handle their own zones
                    await this.buildAndDisplayFileTree(); await this.loadCommitHistory(); await this.loadBranchData();
                 } else if (mergeResult.alreadyMerged) { this.addOutput('Repository already up-to-date.'); }
                 else { this.addOutput('Pull fetch completed, merge status unclear.', 'warn'); await this.buildAndDisplayFileTree(); await this.loadCommitHistory(); await this.loadBranchData(); }
            } else if (fetchResult?.fetchHeadDescription?.includes('already up-to-date')) { this.addOutput('Repository already up-to-date.'); }
             else { this.addOutput('Fetch completed, no new changes detected.', 'log'); }
        } catch (error: any) {
            this.addOutput(`Pull operation failed: ${error.message || error}`, 'error'); console.error("Pull error:", error);
            const statusCode = error.data?.statusCode || error.http?.statusCode;
            if (statusCode === 401) { this.addOutput('Authentication failed (401).', 'error'); this._ngZone.run(() => this.githubAccessToken = null); }
            else if (error.code === 'FastForwardError' || error.name === 'FastForwardError') { this.addOutput('Pull failed: Local branch has diverged. Cannot fast-forward.', 'error'); }
            else if (error.name === 'MergeConflictError') { this.addOutput('Pull failed: Merge conflict (unexpected with fast-forward only).', 'error'); }
        } finally {
    ...
    async pushChanges(): Promise<void> {
        if (!this.activeRepository || !this.cloneDone || this.isLoading || !this.fs || !this.githubAccessToken) { this.addOutput('Push pre-conditions not met.', 'warn'); return; }
        if (this.selectedCommit) { this.addOutput('Cannot push while viewing history.', 'warn'); return; }
        if (this.selectedFile?.isDirty) { this.addOutput('Cannot push: Unsaved changes exist.', 'warn'); return; }

        // Optional: Check if ahead of remote (async ops, no zone needed for internal logic)
        try {
            const currentBranchName = await git.currentBranch({ fs: this.fs, dir: this.dir });
            if (!currentBranchName) { this.addOutput('Cannot push: Detached HEAD state.', 'error'); return; }
            const remoteRef = `refs/remotes/origin/${currentBranchName}`; const localRef = `refs/heads/${currentBranchName}`;
            const localOid = await git.resolveRef({ fs: this.fs, dir: this.dir, ref: localRef }); let remoteOid = null;
            try { remoteOid = await git.resolveRef({ fs: this.fs, dir: this.dir, ref: remoteRef }); } catch(e:any) { if(e.code !== 'NotFoundError') throw e; }
            if (localOid === remoteOid) { this.addOutput('No changes to push.', 'log'); return; }
            this.addOutput(`Local '${currentBranchName}' is ahead. Ready to push.`);
        } catch (statusError: any) { this.addOutput(`Could not determine push status: ${statusError.message}`, 'warn'); }
        ...
        try {
            const currentBranchName = await git.currentBranch({ fs: this.fs, dir: this.dir });
            if (!currentBranchName) throw new Error('Cannot push from detached HEAD state.');

            this.addOutput(`Pushing local branch '${currentBranchName}' to remote 'origin'...`);
            const result: PushResult = await git.push({
                fs: this.fs, http: http, dir: this.dir, url: pushUrl, ref: currentBranchName, remote: 'origin',
            ...
            if (result?.ok && Array.isArray(result.ok) && result.ok.includes('ok') && !result.error) {
                this.addOutput(`Push successful for branch '${currentBranchName}'.`, 'log');
                // Refresh branch data (handles its own zone)
                await this.loadBranchData();
            } else if (result?.error) {
                this.addOutput(`Push failed with remote error: ${result.error}`, 'error');
                if (result.error.toLowerCase().includes('non-fast-forward')) { this.addOutput('Push rejected: Remote has changes. Pull first.', 'error'); }
                else if (result.error.toLowerCase().includes('permission') || result.error.toLowerCase().includes('forbidden')) { this.addOutput('Push rejected: Permission denied (403/401).', 'error'); }
                else { this.addOutput(`Push failed: ${result.error}`, 'error'); }
            } else { this.addOutput(`Push status uncertain. Result: ${JSON.stringify(result)}.`, 'warn'); }
        } catch (error: any) {
            this.addOutput(`Push operation failed: ${error.message || error}`, 'error'); console.error("Push error:", error);
            const statusCode = error.data?.statusCode || error.http?.statusCode;
            if (statusCode === 401) { this.addOutput('Auth failed (401).', 'error'); this._ngZone.run(() => this.githubAccessToken = null); }
            else if (statusCode === 403) { this.addOutput('Auth failed (403).', 'error'); }
            else if (error.name === 'PushRejectedError') { this.addOutput(`Push rejected: ${error.data?.reason || 'Check logs.'}`, 'error'); }
            else if (error.message.includes('detached HEAD')) { this.addOutput(error.message, 'error'); }
        } finally {
    ...
    async getStatus(): Promise<void> {
        if (!this.activeRepository || !this.cloneDone || this.isLoading || !this.fs) { this.addOutput('Status pre-conditions not met.', 'warn'); return; }
        if (this.selectedCommit) { this.addOutput('Status is against HEAD. Deselect commit view.', 'warn'); return; }

        this.setLoading(true, `Status (${this.activeRepository.name})`); // Handles zone
        try {
            const statusMatrix: Awaited<ReturnType<typeof git.statusMatrix>> = await git.statusMatrix({ fs: this.fs, dir: this.dir });
            ...
            if (statusMatrix.length === 0) {
                ...
                statusOutput.push('  St M Path'); statusOutput.push('  -- - ----');
                statusMatrix.forEach(row => {
                    const filepath = row[filepathIdx]; const headStatus = row[headStatusIdx] as GitStatusCode;
                    const workdirStatus = row[workdirStatusIdx] as GitStatusCode; const stageStatus = row[stageStatusIdx] as GitStatusCode;
                    let stagedChar = ' '; let workdirChar = ' ';

                    if (stageStatus === GIT_STATUS.ADDED) stagedChar = 'A';
                    else if (stageStatus === GIT_STATUS.DELETED) stagedChar = 'D';
                    else if (stageStatus === GIT_STATUS.MODIFIED) stagedChar = 'M';

                    // Determine workdir status relative to index
                    if (workdirStatus === GIT_STATUS.ADDED) { // Untracked or Modified-then-readded?
                        if (stageStatus === GIT_STATUS.ABSENT) { stagedChar = '?'; workdirChar = '?'; } // Untracked
                        else { workdirChar = ' '; stagedChar = 'A'; } // Staged as Added, workdir matches index
                    }
                    else if (workdirStatus === GIT_STATUS.DELETED) workdirChar = 'D'; // Deleted in workdir
                    else if (workdirStatus === GIT_STATUS.MODIFIED) workdirChar = 'M'; // Modified in workdir

                    // Combine and filter - only show lines with changes
                    const statusString = `${stagedChar}${workdirChar}`;
                    if (statusString.trim() !== '' || stagedChar === '?') { // Show untracked ('??') and changes
            ...
            this.addOutput(`Failed to get status: ${error.message || error}`, 'error'); console.error("Status error:", error);
        } finally {
    ...
    async viewFileContent(node: TreeNode): Promise<void> {
        if (node.isDirectory || this.isProcessingTree || !this.activeRepository || !this.fs) { return; }

        if (!this.selectedCommit && this.selectedFile?.isDirty) {
            if (!confirm("Discard unsaved changes and view the new file?")) {
                this.addOutput("File view cancelled.", "warn"); return;
            }
             // Discard changes within zone
             this._ngZone.run(() => { if(this.selectedFile) { this.selectedFile.content = this.selectedFile.originalContent; this.selectedFile.isDirty = false; } });
        }
        ...
        this._ngZone.run(() => {
            this.selectedFile = { path: node.path, content: null, originalContent: null, isDirty: false, isLoading: true, error: undefined };
            this.cmOptions = { ...this.cmOptions, readOnly: !!this.selectedCommit };
        });

        try {
            let content: string | null = null; let errorMessage: string | undefined = undefined;
            const extension = node.name.split('.').pop()?.toLowerCase() || ''; const mode = this.getModeForExtension(extension);

            if (this.selectedCommit) { // --- Reading from history ---
                this.addOutput(`Reading file from commit ${this.selectedCommit.oid.substring(0, 7)}...`);
                try {
                    const readResult: ReadObjectResult = await git.readObject({ fs: this.fs, dir: this.dir, oid: this.selectedCommit.oid, filepath: node.path });
                    if (readResult.type === 'blob' && readResult.object instanceof Uint8Array) {
                        try { content = Buffer.from(readResult.object).toString('utf8'); }
                        catch (decodeError: any) { errorMessage = this.handleDecodeError(decodeError); content = null; }
                    } else { errorMessage = `Object is not a file (type: ${readResult.type}).`; }
                } catch (readError: any) { errorMessage = this.handleReadError(readError, true); console.error(`Hist read error: ${node.path}`, readError); }
            } else { // --- Reading from working directory ---
                const fsPath = `${this.dir}/${node.path}`;
                try {
                    const contentBuffer: Uint8Array = await this.fs.promises.readFile(fsPath);
                    try { content = Buffer.from(contentBuffer).toString('utf8'); }
                    catch (decodeError: any) { errorMessage = this.handleDecodeError(decodeError); content = null; }
                } catch (readError: any) { errorMessage = this.handleReadError(readError, false); console.error(`WD read error: ${fsPath}`, readError); }
            }
            ...
            this._ngZone.run(() => {
                ...
                if (this.selectedFile?.path === node.path) {
                    this.selectedFile.content = content; this.selectedFile.originalContent = content;
                    this.selectedFile.isLoading = false; this.selectedFile.isDirty = false;
                    this.selectedFile.error = errorMessage;
                    this.cmOptions = { ...this.cmOptions, mode: mode, readOnly: !!this.selectedCommit };
                } else {
                    // If selection changed while loading, log it but don't update the (now wrong) selectedFile
                    this.addOutput(`File selection changed while loading ${node.path}. Aborting update.`, 'warn');
                }
            });

            if (!errorMessage) { this.addOutput(`Successfully read: ${node.path}${this.selectedCommit ? ' (historical)' : ''}`); }
            else { this.addOutput(`Error reading ${node.path}: ${errorMessage}`, 'error'); }
        } catch (error: any) {
            this.addOutput(`Unexpected error viewing file ${node.path}: ${error.message || error}`, 'error'); console.error("View file unexpected error:", error);
            // Update error state within zone
            this._ngZone.run(() => { if (this.selectedFile?.path === node.path) { this.selectedFile.isLoading = false; this.selectedFile.error = `Unexpected error: ${error.message || error}`; } });
        } finally {
            this.setProcessingTree(false); // Handles zone
        }
    }
    ...
    private handleDecodeError(decodeError: any): string {
        ...
        if (decodeError instanceof Error && (decodeError.message.includes('invalid byte sequence') || decodeError.message.includes('malformed UTF-8'))) {
    ...
    private handleReadError(readError: any, isHistorical: boolean): string {
        ...
        if (readError.code === 'NotFoundError' || readError.name === 'NotFoundError' || readError.code === 'ENOENT') { return `File not found ${context}.`; }
    ...
    private getModeForExtension(extension: string): string {
        switch (extension) {
            case 'js': case 'mjs': case 'cjs': return 'javascript'; case 'ts': return 'text/typescript'; case 'json': return 'application/json'; case 'html': case 'htm': return 'xml'; case 'xml': return 'xml'; case 'css': return 'css'; case 'scss': case 'sass': return 'text/x-scss'; case 'less': return 'text/x-less'; case 'md': case 'markdown': return 'markdown'; case 'java': case 'cs': case 'c': case 'cpp': case 'h': return 'text/x-java'; case 'py': return 'python'; case 'sh': case 'bash': case 'zsh': return 'text/x-sh'; case 'yaml': case 'yml': return 'yaml'; case 'sql': return 'sql'; case 'php': return 'php'; case 'rb': return 'ruby'; case 'go': return 'go'; case 'rs': return 'rust'; case 'pl': return 'perl'; case 'lua': return 'lua'; default: return 'text/plain';
        }
    }
    ...
    onCodeMirrorChange(newContent: string): void {
        ...
        if (this.selectedFile && !this.selectedFile.isLoading && !this.selectedCommit) {
    ...
    async saveFileChanges(): Promise<void> {
        if (!this.selectedFile?.isDirty || this.isSavingFile || !this.activeRepository || !this.fs) { this.addOutput('Save condition not met.', 'warn'); return; }
        if (this.selectedCommit) { this.addOutput('Cannot save while viewing history.', 'error'); return; }
        ...
        try {
            ...
            this._ngZone.run(() => {
                if (this.selectedFile?.path === filePath) { // Check if still relevant
            ...
            this.addOutput(`Error saving/staging ${filePath}: ${error.message || error}`, 'error'); console.error("Save/stage error:", error);
        } finally {
    ...
    async commitChanges(): Promise<void> {
        if (!this.activeRepository || this.isBusy || !this.fs) { this.addOutput('Commit pre-conditions not met.', 'warn'); return; }
        if (this.selectedCommit) { this.addOutput('Cannot commit while viewing history.', 'error'); return; }
        if (this.selectedFile?.isDirty) { this.addOutput('Cannot commit: Unsaved changes exist.', 'warn'); return; }

        const commitMessage = prompt("Enter commit message:");
        if (!commitMessage) {
            this.addOutput('Commit cancelled (no message provided).', 'warn');
            return;
        }

        const authorInfo = { name: 'Browser User', email: 'user@browser.com' };
        this.addOutput(`Attempting to commit: "${commitMessage}"...`);
        this.setLoading(true, `Commit (${this.activeRepository.name})`); // Handles zone

        try {
            const sha = await git.commit({ fs: this.fs, dir: this.dir, message: commitMessage, author: authorInfo });
            this.addOutput(`Commit successful! New commit SHA: ${sha.substring(0, 7)}`, 'log');
            await this.loadCommitHistory(); // Refresh history (handles its own zone)
        } catch (error: any) {
            this.addOutput(`Commit failed: ${error.message || error}`, 'error'); console.error("Commit error:", error);
            if (error.code === 'EmptyCommitError' || error.name === 'EmptyCommitError') {
                this.addOutput('Commit failed: No changes were staged.', 'warn');
            }
        } finally {
            this.setLoading(false, `Commit (${this.activeRepository.name})`); // Handles zone
        }
    }
    ...
    async loadCommitHistory(): Promise<void> {
        if (!this.activeRepository || !this.cloneDone || !this.fs || this.isLoadingHistory) { return; }
        this.addOutput(`Loading commit history (depth: ${this.historyDepth})...`);
        // Update loading state within zone
        this._ngZone.run(() => {
            this.isLoadingHistory = true; this.historyError = null; this.commitHistory = [];
        });

        try {
            const logs: GitLogEntry[] = await git.log({ fs: this.fs, dir: this.dir, depth: this.historyDepth, ref: 'HEAD' });
            // Update history within zone
            this._ngZone.run(() => { this.commitHistory = logs; });
            this.addOutput(`Loaded ${logs.length} commit entries.`);
        } catch (error: any) {
            this.addOutput(`Failed history load: ${error.message || error}`, 'error'); console.error("History load error:", error);
            // Update error state within zone
            this._ngZone.run(() => {
                this.historyError = `Failed: ${error.message || error}`; this.commitHistory = [];
            });
        } finally {
            // Update loading state within zone
            this._ngZone.run(() => { this.isLoadingHistory = false; });
            // No explicit detectChanges needed
        }
    }

    async selectCommit(commit: GitLogEntry | null): Promise<void> {
        if (this.isBusy) { this.addOutput("Wait for current operation.", "warn"); return; }
        if (commit !== null && this.selectedFile?.isDirty) {
            if (!confirm("Discard unsaved changes and view commit?")) { this.addOutput("Commit selection cancelled.", "warn"); return; }
             // Discard changes within zone
             this._ngZone.run(() => { if(this.selectedFile) { this.selectedFile.content = this.selectedFile.originalContent; this.selectedFile.isDirty = false; } });
        }
        ...
        this._ngZone.run(() => {
            ...
            if (commit) {
                this.addOutput(`Selected commit: ${commit.oid.substring(0, 7)} - "${commit.commit.message.split('\n')[0]}"`);
                this.cmOptions = { ...this.cmOptions, readOnly: true };
                this.selectedFile = null; // Reset file selection when changing context
            } else { // Switching back to HEAD
    ...
    viewHead(): void { this.selectCommit(null); } // selectCommit handles async/zone
    formatTimestamp(timestamp: number): string { return this.datePipe.transform(timestamp * 1000, 'medium') || 'Invalid Date'; }


    // --- Branch Management Logic ---
    async loadBranchData(): Promise<void> {
        if (!this.activeRepository || !this.cloneDone || !this.fs || this.isLoading) { return; }
        this.addOutput('Loading branch info...');
        // Reset error within zone
        this._ngZone.run(() => { this.branchError = null; });

        try {
             const [localBranchesList, remoteBranchesList, currentBranchName] = await Promise.all([
                git.listBranches({ fs: this.fs, dir: this.dir }),
                git.listBranches({ fs: this.fs, dir: this.dir, remote: 'origin' }).catch(() => { this.addOutput("Could not list remote branches.", 'warn'); return []; }),
                git.currentBranch({ fs: this.fs, dir: this.dir, fullname: false }).catch(err => { if(err.code === 'DetachedHeadError') { this.addOutput("Detached HEAD state.", 'warn'); return null; } throw err; })
            ]);
            const remoteFiltered = remoteBranchesList.filter(b => b !== 'origin/HEAD').map(b => b.startsWith('origin/') ? b.substring('origin/'.length) : b).sort();
            // Update state within zone
            this._ngZone.run(() => {
            ...
            this.addOutput(`Branches loaded. Current: ${this.currentBranch || 'Detached HEAD'}.`);
        } catch (error: any) {
            this.addOutput(`Branch load failed: ${error.message || error}`, 'error'); console.error("Branch load error:", error);
            // Update error state within zone
            this._ngZone.run(() => {
                this.branchError = `Failed: ${error.message || error}`;
                this.localBranches = []; this.remoteBranches = []; this.currentBranch = null;
            });
        } finally {
            // No explicit detectChanges needed
        }
    ...
    async switchBranch(branchName: string): Promise<void> {
        if (!this.activeRepository || !this.cloneDone || !this.fs || this.isBusy || branchName === this.currentBranch || !branchName) { this.addOutput('Switch branch pre-conditions not met.', 'warn'); return; }
        if (this.selectedCommit) { this.addOutput('Cannot switch branch while viewing history.', 'warn'); return; }
        if (this.selectedFile?.isDirty) {
            if (!confirm(`Discard unsaved changes and switch to '${branchName}'?`)) { this.addOutput('Switch cancelled.', 'warn'); return; }
            // Discard changes within zone
            this._ngZone.run(() => { if(this.selectedFile) { this.selectedFile.content = this.selectedFile.originalContent; this.selectedFile.isDirty = false; } });
        }
        ...
        try {
             ...
             const hasUncommitted = matrix.some(row => row[workdirStatusIdx] !== GIT_STATUS.ABSENT || row[stageStatusIdx] !== GIT_STATUS.ABSENT);
            if (hasUncommitted) { this.addOutput('Cannot switch: Uncommitted changes exist. Commit or stash first.', 'error'); return; }
        } catch (statusError: any) { this.addOutput(`Warn: Could not check status before switch: ${statusError.message}`, 'warn'); }

        this.addOutput(`Switching to branch '${branchName}'...`);
        // Update loading state within zone
        this._ngZone.run(() => { this.isSwitchingBranch = true; this.branchError = null; });
        ...
        try {
            ...
            this._ngZone.run(() => {
                this.selectedCommit = null; this.selectedFile = null;
                this.cmOptions = { ...this.cmOptions, readOnly: false };
            });
            // Reload data - these handle their own zones
            await this.loadBranchData(); await this.loadCommitHistory(); await this.buildAndDisplayFileTree();
        } catch (error: any) {
            this.addOutput(`Switch failed: ${error.message || error}`, 'error'); console.error("Switch branch error:", error);
            // Update error state within zone
            this._ngZone.run(() => { this.branchError = `Checkout failed: ${error.message || error}`; });
            if (error.code === 'CheckoutConflictError') { this.addOutput('Conflict: Commit/stash changes.', 'error'); }
            else if (error.code === 'NotFoundError') { this.addOutput(`Branch '${branchName}' not found.`, 'error'); }
            await this.loadBranchData(); // Reload data even on failure
        } finally {
            // Update loading state within zone
            this._ngZone.run(() => { this.isSwitchingBranch = false; });
    ...
    async createBranch(): Promise<void> {
        if (!this.activeRepository || !this.cloneDone || !this.fs || this.isBusy || !this.newBranchName.trim()) { this.addOutput('Create branch pre-conditions not met.', 'warn'); return; }
        if (this.selectedCommit) { this.addOutput('Cannot create branch while viewing history.', 'warn'); return; }
        const branchToCreate = this.newBranchName.trim();
        if (!/^[^\s~^:?*[\\^@{}]+$/.test(branchToCreate) || branchToCreate.includes('..') || branchToCreate.includes('/.') || branchToCreate.startsWith('/') || branchToCreate.endsWith('/') || branchToCreate.endsWith('.lock')) {
            this.addOutput(`Invalid branch name: "${branchToCreate}".`, 'error');
            this._ngZone.run(() => this.branchError = "Invalid name."); return;
        }
        if (this.localBranches.includes(branchToCreate) || this.remoteBranches.includes(branchToCreate)) {
            this.addOutput(`Branch "${branchToCreate}" already exists.`, 'error');
            this._ngZone.run(() => this.branchError = "Already exists."); return;
        }

        this.addOutput(`Creating & checking out '${branchToCreate}'...`);
        // Update loading state within zone
        this._ngZone.run(() => { this.isCreatingBranch = true; this.branchError = null; });
        this.setLoading(true, `Create Branch (${branchToCreate})`); // Handles zone

        try {
            await git.branch({ fs: this.fs, dir: this.dir, ref: branchToCreate, checkout: true });
            this.addOutput(`Created & switched to '${branchToCreate}'.`);
            // Reset state within zone
            this._ngZone.run(() => {
                this.newBranchName = ''; this.selectedCommit = null; this.selectedFile = null;
                this.cmOptions = { ...this.cmOptions, readOnly: false };
            });
            // Reload data - these handle their own zones
            await this.loadBranchData(); await this.loadCommitHistory(); await this.buildAndDisplayFileTree();
        } catch (error: any) {
            this.addOutput(`Create branch failed: ${error.message || error}`, 'error'); console.error("Create branch error:", error);
            // Update error state within zone
            this._ngZone.run(() => { this.branchError = `Create failed: ${error.message || error}`; });
            await this.loadBranchData(); // Reload data even on failure
        } finally {
            // Update loading state within zone
            this._ngZone.run(() => { this.isCreatingBranch = false; });
    ...
    addOutput(message: string, level: 'log' | 'warn' | 'error' = 'log'): void {
        const timestamp = new Date().toLocaleTimeString(); const logMessage = `[${timestamp}] ${message}`;
    ...
    setLoading(loading: boolean, operation?: string): void {
        ...
        this._ngZone.run(() => {
            ...
            if (operation) {
                ...
                if (!loading) {
                    setTimeout(() => this.addOutput(`Operation ${status}: ${operation}.`), 0);
                } else {
    ...
    buildTreeFromPaths(paths: string[]): TreeNode[] {
        const root: TreeNode = { name: '__root__', path: '', isDirectory: true, children: [], expanded: true };
        paths.forEach((path) => {
            let currentNode = root; const parts = path.split('/');
            parts.forEach((part, index) => {
                ...
                if (!childNode) {
                    childNode = { name: part, path: currentPath, isDirectory: isDirectory, children: isDirectory ? [] : undefined, expanded: false };
                    currentNode.children.push(childNode);
                    // Sort children: directories first, then alphabetically
                    currentNode.children.sort((a, b) => (a.isDirectory === b.isDirectory) ? a.name.localeCompare(b.name) : (a.isDirectory ? -1 : 1));
                } else {
                    ...
                    if (isDirectory && !childNode.isDirectory) {
                        this.addOutput(`Tree build warn: Path '${currentPath}' implies directory, but node '${part}' was previously marked as file. Updating.`, 'warn');
                ...
                if (childNode.isDirectory) {
                    ...
                    this.addOutput(`Tree build error: Attempted to traverse into file path '${childNode.path}' while processing '${path}'.`, 'error');
    ...
    get isBusy(): boolean {
        ...
        return this.isLoading || this.isProcessingTree || this.isSavingFile || this.isLoadingHistory || this.isSwitchingBranch || this.isCreatingBranch || this.isFetchingRepos;
    }
