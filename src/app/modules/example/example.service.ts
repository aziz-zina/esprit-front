...
export class ExampleService {
            ...
            .pipe(
                map((response: string) => {
                    try {
                        ...
                        if (!ghToken) {
                             console.warn('GitHub access_token not found in Keycloak broker response.');
                             return null;
                        }
                        return ghToken;
                    } catch (e) {
                        console.error('Error parsing Keycloak broker response:', e);
                        return null;
                    }
                }),
                catchError(error => {
                    console.error('HTTP Error fetching GitHub token from Keycloak:', error);
    ...
    fetchGithubRepositories(accessToken: string): Observable<GithubApiRepo[]> {
        const apiUrl = 'https://api.github.com/user/repos?sort=updated&per_page=100'; // Get user's repos, sorted, max 100
        const headers = new HttpHeaders({
        ...
        return this._httpClient.get<GithubApiRepo[]>(apiUrl, { headers }).pipe(
            catchError(error => {
                 console.error('HTTP Error fetching GitHub repositories:', error);
